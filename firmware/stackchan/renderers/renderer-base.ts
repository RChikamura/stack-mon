...
export const defaultFaceContext: Readonly<FaceContext> = {
  ...
  theme: {
    ...
    third: [0xb9, 0xff, 0x44] as Color, // 追加
    fourth: [0xff, 0xbb, 0xbf] as Color, // 追加
    fifth: [0xff, 0xa4, 0xa4] as Color, // 追加
  },
}
...
export function copyFaceContext(src: Readonly<FaceContext>, dst: FaceContext) {
  ...
  colorDst[2] = colorSrc[2]
  // 以下追加
  colorDst = dst.theme.third
  colorSrc = src.theme.third
  colorDst[0] = colorSrc[0]
  colorDst[1] = colorSrc[1]
  colorDst[2] = colorSrc[2]
  colorDst = dst.theme.fourth
  colorSrc = src.theme.fourth
  colorDst[0] = colorSrc[0]
  colorDst[1] = colorSrc[1]
  colorDst[2] = colorSrc[2]
  colorDst = dst.theme.fifth
  colorSrc = src.theme.fifth
  colorDst[0] = colorSrc[0]
  colorDst[1] = colorSrc[1]
  colorDst[2] = colorSrc[2]
}
...
export class Layer {
  ...
  #type: 'fill' | 'stroke'
  #lineWidth: number // 追加
  constructor({ colorName = 'primary', type = 'fill', lineWidth = 6 }: LayerProps) { // lineWidthを追加
    this.#renderers = new Map()
    this.#colorName = colorName
    this.#type = type
    this.#lineWidth = lineWidth // 追加
  }
  ...
  render(tick: number, poco: PocoPrototype, face: FaceContext) {
    ...
    const outline =
      this.#type === 'fill'
        ...
        : Outline.stroke(path, this.#lineWidth, Outline.LINECAP_ROUND, Outline.LINEJOIN_ROUND).translate(0, face.breath * 3 ?? 0) // いろいろ追加。端や角の形状をここに書くのはあまり好ましくないと思うが、実装が面倒なので許して
...
export class RendererBase {
  ...
  update(interval = INTERVAL, faceContext: Readonly<FaceContext> = defaultFaceContext): void {
    ...
    if (shouldRender) {
      if (!shouldClear) {
        // ずんだもん表示の時、画面全体で描画(通常は画面の上下左右60pxは描画していない)
        if(config.renderer.type != 'zunf') { // 通常
  ...
  renderFace(tick: number, face: FaceContext, poco: PocoPrototype = this._poco): void {
    ...
    if(config.renderer.type != 'zunf') { // 通常
      ...
      poco.clip(0, 0, poco.width, poco.height)
      poco.fillRectangle(bg, 0, 0, poco.width, poco.height)
      poco.fillRectangle(poco.makeColor(...face.theme.third), 0, 0, poco.width, 20)
    }
